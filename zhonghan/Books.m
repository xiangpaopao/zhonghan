...
- (id)initWithAttributes:(NSDictionary *)attributes {
    ...
    NSString *filename=[plistPath1 stringByAppendingPathComponent:@"BooksReadData.plist"];
...
-(void)setReaded{
    ...
    NSString *plistPath1 = [paths objectAtIndex:0];
    NSString *filename=[plistPath1 stringByAppendingPathComponent:@"BooksReadData.plist"];
    NSMutableArray *bookArr = [[NSMutableArray alloc]initWithContentsOfFile:filename];
    if ([bookArr containsObject:[NSString stringWithFormat:@"%i",_ID]]==NO) {
        [bookArr addObject:[NSString stringWithFormat:@"%i",_ID]];
    }
    [bookArr writeToFile:filename atomically:YES];
    _ifRead = YES;
...
+ (void)globalBooksWithBlock:(void (^)(NSArray *posts, NSError *error))block  page:(NSUInteger)page ts:(NSString*)ts{
    NSString *path = [NSString stringWithFormat:@"api/zhonghan/emags?page_size=%@&page=%d&ts=%@",kApp_PAGE_SIZE,page,ts];
    [[AppAPIClient sharedClient] getPath:path parameters:nil success:^(AFHTTPRequestOperation *operation, id JSON) {
        NSLog(@"globalBooksWithBlock %@",JSON);
        NSArray *postsFromResponse = [[JSON valueForKeyPath:@"response"] valueForKey:@"emags"];
        NSMutableArray *mutablePosts = [NSMutableArray arrayWithCapacity:[postsFromResponse count]];
        for (NSDictionary *attributes in postsFromResponse) {
            Books *books = [[Books alloc] initWithAttributes:attributes];
            if (books != nil) {
                [mutablePosts addObject:books];
            }
        }
        if (block) {
...
+ (void)updateLocalTs:(NSString*)ts{
    ...
    [homeDic setObject:ts forKey:@"emagTs"];
